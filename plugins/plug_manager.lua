...
‏sendDocument(msg.to.id, msg.id_,0, 1, nil, "./plugins/"..string.match (v, "(.*)%.lua") ..".lua", '🗂¦ آلمـلف مـقدم مـن قنآ‏‏هہ MEKOO ¦ֆ \n📡¦ تابع قناة السورس @VPS_OO\n👨🏽‍🔧', dl_cb, nil) end  else
‏if not plugin_exists(matches[2]) then 
‏return '🗂*¦* لا يوجد ملف بهذا الاسم .\n\n'
‏else sendDocument(msg.to.id, msg.id_,0,1,nil,"./plugins/"..matches[2]..".lua",'🗂¦ آلمـلف مـقدم مـن قنآ‏‏هہ MEKOO ¦ֆ \n📡¦ تابع قناة السورس @VSP_OO\n👨🏽‍🔧', dl_cb, nil)
‏end end end
...
‏if not we_sudo(msg)  then return "📛*￤* هذا الاوامر للمطور الاساسي فقط 🛠" end
‏return"👨🏾‍🔧*¦* انتهى دعم هذا السورس بالتحديثات ترقبو السورس الجديد وبالتيجي الجديد في قناة الزعيم @VPS_OO . \n📦"
‏end
